...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($password !== $password2) {
    ...
    // Cek username
    $cek = mysqli_query($conn, "SELECT * FROM user WHERE LOWER(username)=LOWER('$username')");
    if (mysqli_num_rows($cek) == 0) {
      $error = "Username tidak ditemukan!";
    } else {
      $hash = password_hash($password, PASSWORD_DEFAULT);
      mysqli_query($conn, "UPDATE user SET password='$hash' WHERE LOWER(username)=LOWER('$username')");
  ...
  <div class="auth-card">
    ...
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <input type="password" id="password2" name="password2" placeholder="Ulangi password baru" required>
      </div>
